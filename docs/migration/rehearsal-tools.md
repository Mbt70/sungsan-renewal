...
| `tools/migration/file-plan.mjs` | 이전된 글과 첨부 묶음 JSON/JSONL | 첨부 계획 JSON, `g5_board_file` INSERT SQL | `news/free` 대상만 생성, `exclude/intro` 대상과 차단 또는 허용 목록 밖 확장자는 복사 제외 |
...
`attachment-copy-plan.json`은 `copyRecords`, `fileRows`, `blockedRecords`를 포함합니다. `blockedRecords`에는 PHP, PHP7/PHP8, HTML, JS, SVG 계열 파일, `shell.php7`, `shell.php8`, `shell.php.jpg` 같은 실행형 또는 다중 확장자 파일, `.htaccess`, `.user.ini` 같은 서버 설정 파일이 `blocked-extension` 사유로 기록됩니다. `news/free` 서버 허용 목록 밖의 `archive.zip`, `backup.7z` 같은 첨부와 확장자가 없는 파일은 `unsupported-extension` 사유로 기록됩니다.
...
- `attachments.blockedByReason`: `blocked-extension`, `unsupported-extension` 등 첨부 복사 차단 사유별 수
...
- 첨부 복사는 계획 JSON을 먼저 검토한 뒤 SFTP 또는 로컬 스크립트로 수행합니다. `blockedRecords`에 기록된 PHP, PHP7/PHP8, HTML, JS, SVG 계열 파일, `shell.php7`, `shell.php8`, `shell.php.jpg` 같은 실행형 또는 다중 확장자 파일, `.htaccess`, `.user.ini` 같은 서버 설정 파일, `archive.zip`, `backup.7z` 같은 허용 목록 밖 파일, 확장자가 없는 파일은 복사하지 않습니다.

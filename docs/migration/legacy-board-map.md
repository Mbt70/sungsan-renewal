...
- `tools/migration/file-plan.mjs`는 `news/free`로 실제 이전된 글의 첨부만 `data/file/{target_board}` 복사 계획과 `g5_board_file` SQL로 만듭니다. `z6_2`, `z6_3`, `intro`, `exclude` 대상 첨부는 산출물에서 제외하고 PHP, HTML, JS, SVG 계열 파일은 `blockedRecords`로 남깁니다.

...
8. 그누보드 설치 후 `npm run export:setup-sql`로 생성한 `docs/generated/sungsan-setup.sql`을 검토해 적용합니다.
9. `docs/operations/data-htaccess-template.txt` 내용을 그누보드 설치 경로의 `data/.htaccess`로 복사합니다. 배포 zip은 `/data`를 제외하므로 스테이징과 운영 전환 때 모두 직접 적용합니다.
10. 소속(부서·동아리) 목록을 운영 환경에서 조정해야 하면 그누보드 설치 경로의 `data/sungsan.groups.php`를 만듭니다. 이 파일은 Git과 release zip에 넣지 않고 `/data` 백업 대상으로 관리합니다. slug는 영문 소문자, 숫자, 하이픈만 사용합니다.
...
11. 관리자에서 `news` 게시판의 `bo_category_list`가 `공지|행사|자료|규정|활동소식` 기본값 또는 운영자가 정한 소식 종류 목록으로 저장되어 있는지 확인합니다. 소식 종류를 바꾸면 작성/목록 화면의 종류 선택과 필터, 홈 요약 링크의 최근 공지/다가오는 일정/자료와 규정/활동소식 보기 경로가 함께 바뀌어야 합니다.
12. 관리자에서 테마 `sungsan`, 회원 스킨 `sungsan`, 게시판 스킨 `sungsan_news/sungsan_free`, 권한, 분류, 업로드 제한이 의도대로 잡혔는지 확인합니다.
13. 이전 리허설 데이터를 반영합니다. `wr_7=review_required` 글은 운영자 검토 전 공개하지 않습니다.
14. 변환 결과로 redirect CSV와 Apache 초안을 생성하고 운영 전환 전에 샘플 URL을 확인합니다.

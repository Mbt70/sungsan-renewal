...
10. 관리자에서 `news` 게시판의 `bo_category_list`가 `공지|행사|자료|규정|활동소식` 기본값 또는 운영자가 정한 소식 종류 목록으로 저장되어 있는지 확인합니다. 소식 종류를 바꾸면 작성/목록 화면의 종류 선택과 필터가 함께 바뀌어야 합니다.
11. 관리자에서 테마 `sungsan`, 회원 스킨 `sungsan`, 게시판 스킨 `sungsan_news/sungsan_free`, 권한, 분류, 업로드 제한이 의도대로 잡혔는지 확인합니다.
12. 이전 리허설 데이터를 반영합니다. `wr_7=review_required` 글은 운영자 검토 전 공개하지 않습니다.
...
- 검색과 종류 필터 확인
- `bo_category_list` 소식 종류가 작성/목록 화면에 반영되는지 확인

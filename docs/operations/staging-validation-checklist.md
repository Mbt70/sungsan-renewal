...
이 문서는 로컬 검증만으로 확인할 수 없는 Cafe24 스테이징 검수 항목을 운영 전환 전에 남기는 체크리스트입니다. 각 항목은 담당자, 검수일, 상태, 증거를 채웁니다. 상태는 `대기/통과/보류` 중 하나로 기록합니다.
...
| `free` 게시판 목록은 비회원에게 제목만 노출하고 작성자, 날짜, 조회수는 로그인 후 확인됨 |  |  | 대기 |  |  |
...
통과 항목은 화면 캡처 파일명, 명령 출력 위치, 테스트 계정 역할, 확인한 URL 또는 게시글 번호처럼 재현 가능한 증거를 남깁니다. 보류 항목은 보류 사유, 재검수 담당자, 재검수일을 메모에 남긴 뒤 다시 확인합니다.

보류 항목이 있으면 운영 전환하지 않습니다.
